...
async function buildThemeCss({ entry, label }) {
  ...
  // xp.css@0.2.6 ships a compact `not(hover)` media feature that older CSS
  // parsers accepted but Vite 8's Lightning CSS correctly rejects. Preserve
  // the intent while emitting standards-compliant media query syntax.
  root.walkAtRules('media', (rule) => {
    if (rule.params === '(not(hover))') rule.params = '(not (hover))';
  });
