<script lang="ts">
  ...
  // dsui-gm-proto is used to debug runtimes, so it gates on its required NAPs
  ...
  onMount(() => {
    ...
    void waitForPublicKey(identity, {
      ...
      onError: (err) => console.error('[dsui-gm-proto/App] getPublicKey failed:', err),
    }).then((pk) => {
...
<div class="app h-screen w-screen overflow-hidden bg-base-100 text-base-content font-mono">
  {#if report == null}
    <div class="flex items-center justify-center h-full">
      <p class="text-base-content/60 text-sm animate-pulse">checking runtime…</p>
    </div>
  ...
  {:else}
    <div class="flex flex-col h-full">
      ...
      <div class="flex-1 min-h-0">
        ...
        {:else}
          <div class="flex items-center justify-center h-full">
            <p class="text-base-content/60 text-sm">not logged in</p>
          </div>
        {/if}
      </div>
    </div>
