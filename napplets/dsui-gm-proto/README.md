# DaisyUI GM Proto napplet

A DaisyUI-themed copy of the "GM" inbox napplet. It surfaces the **GM / good morning** notes posted by
...
dsui-gm-proto doubles as a **runtime/shell debugging napplet**, so before it
...
```bash
pnpm --filter @napplelets/dsui-gm-proto dev        # vite dev server (port 5185)
pnpm --filter @napplelets/dsui-gm-proto test       # vitest
pnpm --filter @napplelets/dsui-gm-proto verify     # test + type-check + build
pnpm --filter @napplelets/dsui-gm-proto test:conformance
```
...
The manifest (`d` tag `dsui-gm-proto`) is build-signed by `@napplet/vite-plugin`;
